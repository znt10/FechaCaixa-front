  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-theme-base p-6 transition-colors">
      <div className="w-full max-w-md rounded-[24px] border border-theme-border bg-theme-card p-8 text-center shadow-sm">
        <span className="mb-3 block text-[11px] font-black uppercase tracking-[4px] text-theme-acao">
          FechaCaixa
        </span>
          ...
          className={`mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full text-2xl ${
            status === "sucesso"
              ...
              : status === "erro"
                ...
                : "bg-theme-acao/15 text-theme-acao"
          }`}
        >
        ...
        {status !== "confirmando" && (
          <button
            ...
            className="mt-8 w-full rounded-lg bg-theme-acao px-6 py-3 text-sm font-bold text-white shadow-md transition hover:bg-theme-acao-hover active:scale-[0.98]"
          >
            Ir para o login
          </button>
        )}
